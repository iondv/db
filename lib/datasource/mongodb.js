...
const { DataSource } = require('@iondv/db-contracts');
const { log: { LoggerProxy } } = require('@iondv/commons');
const { utils: { empty }, IonError } = require('@iondv/core');
const { Iterator } = require('@iondv/commons-contracts');
...
const Errors = require('../errors/data-source');
          ...
          try {
            ...
            _this.client.connect((err) => {
              ...
              try {
                ...
                _this.busy = false;
                _this.isOpen = true;
                log.info(format(t('Connected to database: %s'), _this.db.s.namespace.db));
